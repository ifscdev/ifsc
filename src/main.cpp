...
#if defined(NDEBUG)
# error "Inclusivefinancesyscoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Inclusivefinancesyscoin Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Inclusivefinancesyscoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Inclusivefinancesyscoin: find block wanted by given orphan block
...
// Inclusivefinancesyscoin: find last block index up to pindex
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal Inclusivefinancesyscoin miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Inclusivefinancesyscoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Inclusivefinancesyscoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Inclusivefinancesyscoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Inclusivefinancesyscoin: track money supply and mint amount info
...
// Inclusivefinancesyscoin: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // Inclusivefinancesyscoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Inclusivefinancesyscoin: compute stake entropy bit for stake modifier
    ...
    // Inclusivefinancesyscoin: compute stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Inclusivefinancesyscoin: check proof-of-stake
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // Inclusivefinancesyscoin: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// Inclusivefinancesyscoin: attempt to generate suitable proof-of-stake
...
{
    RenameThread("Inclusivefinancesyscoin-loadblk");
