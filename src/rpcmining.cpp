...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Inclusivefinancesyscoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Inclusivefinancesyscoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Inclusivefinancesyscoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Inclusivefinancesyscoin is downloading blocks...");
